...
def main(cfg):
    ...
    _spec = importlib.util.spec_from_file_location("navrl_train_utils", _UTILS_PATH)
    _navrl_train_utils = importlib.util.module_from_spec(_spec)
    _spec.loader.exec_module(_navrl_train_utils)
    evaluate = _navrl_train_utils.evaluate
    ...
    # Enable deprecated Isaac Sim extensions that NavRL/OmniDrones need
    from omni.isaac.core.utils.extensions import enable_extension
    for _ext in ["omni.isaac.ui", "omni.isaac.debug_draw", "omni.isaac.cloner", "omni.isaac.core_nodes", "omni.isaac.sensor", "omni.isaac.range_sensor", "omni.isaac.dynamic_control"]:
        try:
            enable_extension(_ext)
        except Exception as _e:
            print(f"Warning enabling {_ext}: {_e}")
    ...
    if "resume_checkpoint" in cfg and cfg.resume_checkpoint is not None:
        print(f"[NavRL]: loading checkpoint from {cfg.resume_checkpoint}", flush=True)
        policy.load_state_dict(torch.load(cfg.resume_checkpoint, map_location=cfg.device))
    ...
    for i, data in enumerate(collector):
        ...
        if i % cfg.save_interval == 0:
            ...
            print("[NavRL]: model saved at training step:", i, "->", ckpt_path, flush=True)
